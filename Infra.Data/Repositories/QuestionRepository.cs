...
namespace Infra.Data.Repositories
{
    public class QuestionRepository(ApplicationDbContext context):IQuestionRepository
    {
        ...
        {
            return await context.QuestionLikes.FirstOrDefaultAsync(u => u.ProductId == productId && u.UserId == userId);
        }

        public async Task<List<QuestionAnswer>> GetQuestionAnswers(int questionId)
        {
            return await context.QuestionAnswers.Where(u =>
                u.IsClosed == false && u.ProductId == questionId && u.IsConfirmed == true).ToListAsync();
        }
