...
public class DiscountService(IDiscountRepository repository) : IDiscountService
{
    ...
    public async Task UpdateAsync(int id, DiscountEditViewModel viewModel)
    {
        ...
        discount.IsActive = viewModel.IsActive;
    ...
    public async Task AssignUserDiscountAsync(List<int> userIds, int discountId)
    {
        ...
        foreach (var userId in usersToRemove) await repository.RemoveUserDiscountAsync(userId, discountId);

        foreach (var userId in usersToAdd) await repository.AssignUserDiscountAsync(userId, discountId);
        await repository.SaveChangesAsync();
    }

    public async Task<List<UserDiscountViewModel>> GetUserGiftCodes(int userId)
    {
        return await repository.GetUserGiftCodesAsync(userId);
    }
    ...
    public async Task AssignProductDiscountAsync(List<int> productIds, int discountId)
    {
        ...
        foreach (var productId in productsToRemove) await repository.RemoveProductDiscountAsync(productId, discountId);

        foreach (var productId in productsToAdd) await repository.AssignProductDiscountAsync(productId, discountId);
        await repository.SaveChangesAsync();
    }
